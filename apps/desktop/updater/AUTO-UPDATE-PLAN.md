...
- ✅ **版本号单一真源 + `bun run bump` 一键改版**：已实现、已推送 main，当前版本 `0.1.11`。
...
- `apps/desktop/tauri.conf.json` **已删除 `version` 字段** → Tauri 自动继承 workspace 版本（HangCore Agent 首版安装包名随之变 `HangCore Agent_0.1.0_x64-setup.exe`）。
...
3. **托管：GitHub Releases**（`latest.json` 与各平台安装包都作为 Release 资产）。仓库 = `git@github.com:Alvinzero/HangCore-Agent.git`。
...
- **次要不一致**：后端 `VersionCheckService` 默认仓库是 `Alvinzero/HangCore-Agent`（`nomifun-system/src/version.rs:5`），与实际仓库 `Alvinzero/HangCore-Agent` 不一致；本方案走 tauri-plugin-updater，不依赖它，但若以后启用该路径需对齐。
...
- `tauri.conf.json` → `plugins.updater.endpoints` 改为 GitHub Releases 的 `latest.json` 稳定地址：
  `https://github.com/Alvinzero/HangCore-Agent/releases/latest/download/latest.json`
...
4. **确认发布仓库** = `Alvinzero/HangCore-Agent`（默认按此）。
...
- Windows 发布路径输出 `HangCore Agent_<version>_x64-setup.exe`；mac/linux 包不在 HangCore Agent 首版范围内。

---

## 7. 校验当前状态

- 当前版本：`0.1.11`（`Cargo.toml [workspace.package].version`；`tauri.conf.json` 无 version 字段、靠继承）。
...
- `tauri.conf.json` → `plugins.updater.endpoints` 改为 `https://github.com/Alvinzero/HangCore-Agent/releases/latest/download/latest.json`。
...
4. 仓库：`Alvinzero/HangCore-Agent`（与 `git remote` 一致）。
...
> 注：后端 `nomifun-system/src/version.rs` 那套旧 `VersionCheckService`（默认仓库 `Alvinzero/HangCore-Agent`）与本方案**无关**（本方案不经它）；前端更新检查已全部走 tauri-plugin-updater。如未来要启用该后端路径再对齐其默认仓库。
...
bun run build:win --config apps/desktop/tauri.updater.conf.json   # 产 HangCore Agent_<ver>_x64-setup.exe + .sig
