...
- 当默认地址不可达时，adapter 会优先使用 `KUN_SOURCE_DIR` 或同级 Kun 源码 checkout 启动真实 Kun runtime；如果没有可用源码，再尝试全局 `kun` 命令。
...
1. 确认本机有可用 Kun runtime：推荐保留 Kun 源码 checkout，并用 `KUN_SOURCE_DIR=<Kun checkout>` 指向它；也可以安装 Kun CLI 并确保 `kun` 命令在系统 PATH 中可用。
...
- 如果 Kun 源码和 Kun CLI 都不可用，会在会话启动时报出明确的自动启动失败原因。
