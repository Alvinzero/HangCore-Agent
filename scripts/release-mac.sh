...
commit_paths() {
  ...
  git -c user.name=hangcore -c user.email=hangcore@users.noreply.github.com commit -m "$message"
}
...
Sig="$Tar.sig"
Dmg="dist/desktop/HangCore Agent_${TargetVersion}_universal.dmg"
...
if [[ "$NoPush" -eq 1 ]]; then
  ...
  echo "  推送      : 开启 (author=hangcore -> origin main)"
fi
...
if [[ -n "${NotesContent//[[:space:]]/}" ]]; then
  NotesTmp="$(mktemp "${TMPDIR:-/tmp}/hangcore-relnotes-${TargetVersion}.XXXXXX.md")"
...
if [[ "$Mode" == "CREATE" ]]; then
  if [[ "$NoPush" -eq 1 ]]; then
    ...
    echo "▶ 提交并打 tag ${Tag}（author=hangcore）..."
  ...
  if [[ "$NoPush" -eq 1 ]]; then
    ...
    echo "▶ 提交 latest.json 回 main（author=hangcore）..."
