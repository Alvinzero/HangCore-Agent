...
#
# 前提（一次性配好即可反复用）：
#   1) apps/desktop/signing/hangcore-updater.key —— updater 私钥，
...
$Repo        = 'Alvinzero/HangCore-Agent'
$Triple      = 'x86_64-pc-windows-msvc'
$KeyFile     = 'apps/desktop/signing/hangcore-updater.key'
...
# ── updater 私钥 ─────────────────────────────────────────────────────────────
if (-not (Test-Path $KeyFile)) { Fail "缺少 updater 私钥 $KeyFile（从密钥库拷入，必须与 tauri.conf.json 内嵌 pubkey 匹配）。" }
...
if ($NoPush) { Write-Host "  推送      : -NoPush（见脚本头部语义说明）" } else { Write-Host "  推送      : 开启 (author=hangcore -> origin main)" }
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
$NotesTmp = $null
if ($NotesContent) {
  $NotesTmp = Join-Path $env:TEMP ("hangcore-relnotes-$TargetVersion.md")
...
function Commit-Latest($message, $extraPaths) {
  ...
  git -c user.name=hangcore commit -m $message
...
function Push-Main([string]$tagToMove) {
  ...
  git -c user.name=hangcore pull --rebase origin main
...
if ($Mode -eq 'CREATE') {
  if ($NoPush) {
    ...
    Write-Host "▶ 提交并打 tag $Tag（author=hangcore）..."
  ...
  if ($NoPush) {
    ...
    Write-Host "▶ 提交 latest.json 回 main（author=hangcore）..."
